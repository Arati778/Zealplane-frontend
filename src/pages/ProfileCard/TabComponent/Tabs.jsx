  ...
  const contentList = {
    ...
    thoughts: thoughts.length > 0 ? (
      ...
      <div>
        ...
        <button className="comic-button" onClick={() => navigate('/forum/create-post')}>
          Create a Thought
        </button>
      </div>
    ),
      ...
      const fetchThoughts = async () => {
        try {
          ...
          if (username) { // Ensure username is not empty
            const response = await axios.get(`${apiBaseUrl}/posts/user/${username}`); // Adjust endpoint as necessary
